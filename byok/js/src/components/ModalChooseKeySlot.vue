...
export default {
  ...
  methods: {
    ...
    statusClass: function (key) {
      ...
      } else if (this.selectSymMode) {
        if (isSymmetric(key.type)) {
          return "green";
        } else {
          return "red";
        }
    ...
    selectDisabled: function (key) {
      if (this.slot !== null && this.slot !== this.key.slot) {
        return false;
      } else if (this.selectKekMode) {
        return !isKek(key);
      } else if (this.selectSymMode) {
        return !isSymmetric(key.type);
      } else if (this.selectPrivateMode) {
