...
@section DESCRIPTION
Resources and intents for byok
"""
...
from lib.utils.fx_decorators import __dataclass_transform__, singledispatchmethod
...
class BasePaginationIntent(Model):
    page: int = field(required=False,
                           load_default=1,
                           description='Page number',
                           validate=validate.Range(1))
    pageCount: int = field(required=False,
                                load_default=25,
                                description='Results per page',
                                validate=validate.Range(1, 100))


class BasePaginationResponse(Model):
    page: Optional[int] = field(description='Current page', init=False, example=1)
    nextPage: Optional[int] = field(description='Next page', init=False, example=1)
    pageCount: Optional[int] = field(description='Results per page', init=False, example=10)
    totalPages: Optional[int] = field(description='Number of pages', init=False, example=1)
    totalItems: Optional[int] = field(description='Number of results', init=False, example=1)

    def update_pagination(self) -> None:
        """Deduce missing pagination values"""
        page = getattr(self, 'page', None)
        next_page = getattr(self, 'nextPage', None)
        page_count = getattr(self, 'pageCount', None)
        total_pages = getattr(self, 'totalPages', None)
        total_items = getattr(self, 'totalItems', None)

        # total pages is the ceil of items/(items/page)
        if not total_pages and total_items is not None and page_count:
            total_pages = -(-total_items // page_count)

        # total items is how many objects we have if we have the last page
        if not total_items and page and page == total_pages and page_count:
            for attr in vars(self).values():
                if isinstance(attr, (list, tuple)):
                    total_items = len(attr) + page_count * (total_pages - 1)
                    break

        if not next_page and page and total_pages:
